...
from utils.loss_utils import l1_loss, ssim, compute_depth_loss, compute_rank_loss, compute_continue_loss, segment_loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, depth_loss_choice):
    ...
    gaussians = GaussianModel(dataset.sh_degree, dataset.num_class) # 首先实例化3d高斯
    ...
    # 检测分割图是否准备就绪
    if scene.getTrainCameras()[0].seg is not None and dataset.using_seg:
        print("Segment map Ready！")
    else:
        assert dataset.using_seg == False, "segment map is not exist, so using seg must close"
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        image, viewspace_point_tensor, visibility_filter, radii, depth, segment = \
            render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"], render_pkg["depth"], render_pkg["segment"]
        ...
        # add seg loss
        if viewpoint_cam.seg is not None and dataset.using_seg:

            # segment logic size: [c, h, w]
            gt_segment = viewpoint_cam.seg.cuda()
            '''
            # label trans 这里是debug
            lable = torch.nn.functional.one_hot(gt_segment.squeeze(0).type(torch.long)).permute(2, 0, 1)
            # seg_loss = segment_loss(segment.unsqueeze(0), gt_segment.type(torch.long))
            seg_loss = l1_loss(torch.sigmoid(segment), lable)
            print(seg_loss)
            '''
            seg_loss = segment_loss(segment.unsqueeze(0), gt_segment.type(torch.long))
            # print(seg_loss)
            loss = loss + seg_loss * opt.lambda_segment
        ...
        with torch.no_grad():
            ...
            elif viewpoint_cam.seg is not None and dataset.using_seg:
                training_report_add_segment(tb_writer, iteration, Ll1, seg_loss, loss, l1_loss, iter_start.elapsed_time(iter_end),
...
def training_report_add_segment(tb_writer, iteration, Ll1, seg_loss, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    if tb_writer:
        tb_writer.add_scalar('train_loss_patches/l1_loss', Ll1.item(), iteration)
        tb_writer.add_scalar(f'train_loss_patches/segment_loss', seg_loss.item(), iteration)
        tb_writer.add_scalar('train_loss_patches/total_loss', loss.item(), iteration)
        tb_writer.add_scalar('iter_time', elapsed, iteration)
