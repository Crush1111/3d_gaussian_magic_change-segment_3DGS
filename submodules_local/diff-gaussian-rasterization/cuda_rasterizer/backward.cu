...
renderCUDA(
	...
	const float* __restrict__ dL_dpixels_segments,
	...
	float* __restrict__ dL_dsegments,
...
)
{
	...
	// segment
	__shared__ float collected_segments[S * BLOCK_SIZE];
	...
	// segment
    float accum_segment_rec[S] = { 0 };
	float dL_dpixel_segment[S];
	...
	if (inside) {
        ...
        for (int i = 0; i < S; i++)
			dL_dpixel_segment[i] = dL_dpixels_segments[i * H * W + pix_id];
	}

	float last_alpha = 0;
	float last_color[C] = { 0 };
	float last_segment[S] = { 0 };
	float last_depth = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			// segments
            for (int i = 0; i < S; i++)
				collected_segments[i * BLOCK_SIZE + block.thread_rank()] = segments[coll_id * S + i];
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// segment
			const float dclass_dsegment = alpha * T;
			...
			{
				const float c_s = collected_segments[ch * BLOCK_SIZE + j];
				// Update last color (to be used in the next iteration)
				accum_segment_rec[ch] = last_alpha * last_segment[ch] + (1.f - last_alpha) * accum_segment_rec[ch];
				last_segment[ch] = c_s;

				const float dL_dclass = dL_dpixel_segment[ch];
				dL_dopa += (c_s - accum_segment_rec[ch]) * dL_dclass;
				// Update the gradients w.r.t. color of the Gaussian.
				// Atomic, since this pixel is just one of potentially
				// many that were affected by this Gaussian.
				atomicAdd(&(dL_dsegments[global_id * S + ch]), dclass_dsegment * dL_dclass);
			}
...
void BACKWARD::render(
	...
	const float* dL_dpixel_segments,
	...
	float* dL_ddepths)
{
	renderCUDA<NUM_CHANNELS, NUM_CLASS> << <grid, block >> >(
		ranges,
		point_list,
		W, H,
		bg_color,
